...
LPCTSTR szFileName = TEXT("nbb.bmp");
...
void ShowText()
{
	HFONT	hOldFont;
	RECT	rTextRect;
	wchar_t szInfoText[256];  // Change to wchar_t
	...
	DrawTextW(hMemDC, szFileName, wcslen(szFileName), &rTextRect, DT_SINGLELINE | DT_CENTER | DT_VCENTER); // Use DrawTextW

	wsprintfW(szInfoText, L"Самойлов Александр\nReal width-%d, Real height-%d\n Virtual width-%d, Virtual height-%d", nBmpWidth, nBmpHeight, nVBmpWidth, nVBmpHeight); // Use wsprintfW

	SetRect(&rTextRect, 0, nScreenY - nTopDown, nScreenX, nScreenY);
	DrawTextW(hMemDC, szInfoText, wcslen(szInfoText), &rTextRect, DT_CENTER); 
